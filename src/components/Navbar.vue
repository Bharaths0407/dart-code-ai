...
<template>
    <header class="top-0 bg-background z-50 sticky">
        <div class="relative flex items-center justify-between max-w-7xl mx-auto px-5 py-5">
            ...
            <!-- Navigation Links -->
            <nav class="hidden lg:flex space-x-15 absolute left-1/2 transform -translate-x-1/2 items-center  mb-1 pl-2"
                aria-label="Primary navigation">
                <router-link v-for="(link) in navLinks" :key="link.label" :to="link.href" :class="[
                    'relative font-semibold transition-colors  duration-300  transform hover:scale-105 before:absolute before:-bottom-1 before:left-0 before:w-0 before:h-0.5 before:bg-[#42389E] before:transition-all before:duration-300 hover:before:w-full',
                    route.path === link.href
                        ? 'text-[#42389E] before:w-full'
            ...
            <!-- Mobile Menu Button -->
            <Button class="lg:hidden p-2 border-none" variant="outlineHover" size="icon" @click="toggleMenu"
                aria-label="Toggle Menu" :aria-expanded="isMenuOpen.toString()" aria-controls="mobile-menu">
                <img :src="menuIcon" alt="Menu" class="size-10" />
            </Button>
        </div>
                ...
                <nav aria-label="Mobile Navigation">
                    <ul class="flex flex-col space-y-4">
                        <li v-for="(link) in navLinks" :key="link.label">
                            <router-link :to="link.href" :class="[
